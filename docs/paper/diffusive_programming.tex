...
Diffusers decide how to distribute the method calls based on a strategy (\textsf{Strategy}) that returns an end-point at each request. The strategy can be implemented to take into account the the load on its server, weighting factors for individual end-points, etc. \emph{Diffusive} provides several \textsf{Strategy} implementations.
