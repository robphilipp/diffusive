...
\subsection{Learning}
All servers are not created equal. For the purposes of distributed computing, they differ in three important areas:
\begin{enumerate}
\item Execution capacity
\item File input-output (I/O) capacity
\item Network I/O capacity
\end{enumerate}
For example, for CPU intensive tasks, a server that is running a quad quad-core processor should receive substantially more tasks than a server running a single dual-core processor\footnote{Assuming they have a similar clock speed and instruction set and aren't memory or I/O bound.}.
