...
%During the creation of a remote diffuser, the remote diffuser's class paths are configured. This is represented by the first step shown in figure \ref{fig:diffusing}. In order to execute the task sent to it, the remote diffuser (\textsf{Diffuser 2}) needs to load \textsf{Class} objects related to that task. Because \textsf{Diffuser 1} (the application-attached diffuser, in this case) contains the task and it's \textsf{Class} objects, it passes the URI of its web resource (as a class path) along withe the request to create \textsf{Diffuser 2}., from the location at which it was called, the class paths are specified as URIs. Typically, when a diffuser wishes to diffuse a method call, it requests an end-point, and checks to see if that end-point contains a diffuser that matches the diffusive method signature of the method it wishes to diffuse. When the remote end-point doesn't contain the required diffuser, the requesting diffuser requests that the remote diffuser be created. Because the requesting diffuser has the \textsf{Class} objects needed to execute the method, it must  be able to provide those \textsf{Class} objects when requested by the remote diffuser. And therefore, the requesting diffuser will pass it's own URI to the remote diffuser as a class path. In other words, the diffuser requesting the creation of a remote diffuser will act as the source for the remote diffuser's RESTful class loading calls\footnote{This is \emph{Diffusive's} default behavior. However, it is possible to configure diffusive to provide a common URI to all remote diffusers, instead of it's own URI, such that a common area exists holding all the class information. In this case, all the remote diffusers' RESTful class loaders would look to that common location for the serialized \textsf{Class} objects it needs.}.
...
Recall that in \emph{Diffusive}, a remote diffuser is managed by RESTful diffuser server. A RESTful diffuser servers (\textsf{RestfulDiffuserServer}) act as servers to which methods can be diffused. The RESTful diffuser servers also act as clients when diffusing methods to other remote diffusers, or when other diffusers request \textsf{Class} objects from their class path. And therefore, all remote diffusers look alike. Figure \ref{fig:restful_diffuser_server} shows the basic elements contained in a RESTful diffuser server and depicts a typical interaction. In this scenario, the \textsf{RestfulDiffuserServer} receives a request to execute a method (task). The \textsf{RestfulDiffuserServer}, a web server, forwards the request to the web resource (JSR-311 web resource) which binds to the \textsf{execute(...)} method of that resource. The execute method looks up the appropriate diffuser based on the diffuser method signature, which is part of the its URI, and then forwards the method to that diffuser. The diffuser 

Each diffuser contains a list of end-points to which it can diffuse code, a list of class paths from which it can load \textsf{Class} objects, and a \textsf{Strategy} which determines how the end-points for executing the next task are selected. It is the combination of these that allows diffusers to be connected in arbitrary way, as long as that arbitrary way can be represented by a directed graph.

% create a figure that shows the diffuser, and the two ways in which the diffuser is managed: app-attached; remote restful diffuser server.
