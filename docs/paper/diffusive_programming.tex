...
A natural extension of the single-layered diffuser pattern is a multi-layered diffuser pattern. Two such patterns are shown in figure \ref{fig:topology_multi_layer}. However, unless the implementation of the diffusive principles allows nested method \emph{marking}, such network designs do not appear to provide any benefit over a single layer. Furthermore, in practice nested method marking is difficult to implement in a generic manner. If fact, it is not part of the reference implementation.
