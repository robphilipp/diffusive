...
public class ThreadedVolumeCalc extends VolumeCalc {
	...
	public List< Double > calcVolumes( final int numSimulations, final long maxIterations )
	{
		...
		final ExecutorService executor = Executors.newFixedThreadPool( Math.min( numSimulations, 1000 ) );
		...
		{
			final int iteration = i;	// the iteration number must be final to be seen by inner class
	...
	public static void main( final String...args )
	{
		...
		final ThreadedVolumeCalc calc = new ThreadedVolumeCalc( cube, new Cube( 4.0, 4.0, 4.0, 4.0 ) );
